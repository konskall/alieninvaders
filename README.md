...
[![Play Now](https://img.shields.io/badge/▶_PLAY_NOW-Live_Demo-32B8C6?style=for-the-badge)](https://konskall.github.io/alieninvaders/)
&nbsp;
...
- 🏆 **Global leaderboard** (Firebase Realtime Database, top 50) with localStorage fallback and one-tap **score sharing**.
...
Scores are saved to a **Firebase Realtime Database** (region `europe-west1`) and the **top 50** are displayed live in a scrollable list, with 🥇🥈🥉 medals for the podium and each entry showing the player's name, reached level, date, and score. If Firebase is unavailable, the game transparently falls back to **localStorage** so your best run is never lost.
...
> 🔒 The Firebase web config lives in `src/managers/LeaderboardManager.js`. Web API keys are *designed to be public* for client-side Firebase apps — access is governed by the database's security rules, not by hiding the key.
...
Built deliberately with **zero dependencies to install and no build step** — just static files and native ES modules.

| Area | Technology |
|---|---|
| **Language** | Vanilla JavaScript (ES2015+), split into **19 ES modules** under `src/` |
...
| **Styling** | Hand-written CSS design system, split across `css/` + Google Fonts (*Orbitron*, *Russo One*) |
...
```
alieninvaders/
├── index.html                 # Markup + <script type="module" src="src/main.js"> + css/ links
├── src/                       # Game engine (ES modules)
│   ├── main.js                # Bootstrap (creates the Game on load)
│   ├── game.js                # The Game orchestrator
│   ├── config.js              # Tunables: difficulty, waves, CONFIG
│   ├── utils.js               # randomRange, distance, GradientCache
│   ├── gallery.js             # Artwork gallery
│   ├── entities/              # Player, Enemy, Boss, Bullet, HomingBullet,
│   │                          #   Particle, Shockwave, FloatingText, Star, BonusPickup
│   └── managers/              # Leaderboard, Sound, Music, Vibration
├── css/                       # Styles, split into ordered files (01-tokens … 07)
├── Gallery/                   # Concept artwork (PNG)
├── docs/                      # Design spec + implementation plan
├── og-alien.png               # Social share / hero image
├── site.webmanifest           # PWA manifest
├── netlify.toml               # Netlify hosting config
└── favicon.* / web-app-manifest-*.png
```
...
No toolchain required — it's static files. Because the game uses **native ES modules** (and loads Firebase over HTTP), serve the folder rather than opening `index.html` from disk:
...
> ⚠️ Opening `index.html` directly via `file://` will **not** work — browsers block ES module loading from the filesystem. Use a local server. An internet connection is also needed for Google Fonts and the Firebase leaderboard; core gameplay works offline.
...
- **GitHub Pages** — enabled on this repo (branch `main`, root). Live at: **https://konskall.github.io/alieninvaders/**
...
**[▶ Play Alien Invaders](https://konskall.github.io/alieninvaders/)**
